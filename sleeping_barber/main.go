...
func main() {
	...
	//create data structure barbershop
	barbershop := Barbershop{
		ShopCapacity:    seatingCapacity,
		HairCutDuration: cutDuration,
		NumberOfBarbers: 0, // initial value -adding it later
		BarbersDoneChan: doneChan,
		ClientsChan:     clientsChan,
		Open:            true,
	}
	color.Green("The barbershop is open for the day:")
	//add barbers
	barbershop.Add("Frank")
	barbershop.Add("Tom")
	barbershop.Add("Kelly")
	barbershop.Add("Jake")
	barbershop.Add("John")
	...
	go func() {
		for {
			...
			case <-shopClosing:
				return
			case <-time.After(time.Millisecond * time.Duration(randomMilliseconds)): // client arrives
	...
	//block until the barbershop closed
	<-closed
}
